// Next.js API route voor elektrische berekeningen
import { NextRequest, NextResponse } from 'next/server';

// Mock elektrische berekeningen data
...
export async function GET(
  ...
  try {
    ...
    const electrical = electricalData.find(e => e.projectNumber === projectNumber);
    
    if (!electrical) {
      return NextResponse.json({
        success: false,
        error: 'Electrical calculation not found'
      }, { status: 404 });
    }
    ...
    console.error('Error fetching electrical calculation:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to fetch electrical calculation'
    }, { status: 500 });
  }
}
